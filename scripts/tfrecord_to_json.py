...
class TFRecordToJSONConverter:
    def __init__(self, tfrecord_dir, output_dir, dataset_name='crello'):
        ...
        self.sequence_features = {
            ...
            'ratio_w_to_h': tf.io.FixedLenSequenceFeature([], tf.float32),
            ...
            # 内容哈希
            'image_hash': tf.io.FixedLenSequenceFeature([], tf.string),
            'text_hash': tf.io.FixedLenSequenceFeature([], tf.string),
    ...
    def convert_split(self, split='train'):
        ...
        if error_count > 0:
            print(f"⚠ 跳过了 {error_count} 个解析失败的样本")
