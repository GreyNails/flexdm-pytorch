"""
PyTorch版本的MFP模型演示和可视化（完整修复版）
支持SVG转PNG并正确处理坐标归一化和类型映射
"""
import json
import itertools
import logging
...
def create_comparison_image(
    ...
    """创建对比图（多个样本并排）"""
    if not HAS_CAIROSVG:
        logger.warning("需要cairosvg创建对比图")
        return
...
def main():
    ...
    # 9. 可视化重建
    logger.info(f"\n8. 开始可视化 - 任务: {config.target_task}")
    logger.info(f"   列名: {config.column_names[config.target_task]}")
