...
class TFToPyTorchConverter:
    ...
    def convert_to_pytorch(self):
        ...
        for tf_name, tf_tensor in tf_weights.items():
            # 跳过优化器状态（只保留模型权重）
            if 'optimizer' in tf_name.lower():
                continue
            
            # 跳过特殊标记
            if '_CHECKPOINTABLE_OBJECT_GRAPH' in tf_name:
                continue
            
            # 处理不同类型的数据
            try:
                # 检查是否为bytes或其他非numpy类型
                if isinstance(tf_tensor, bytes):
                    print(f"  ⚠ 跳过bytes类型: {tf_name}")
                    continue
                
                # 转换为numpy数组
                if not isinstance(tf_tensor, np.ndarray):
                    tf_tensor = np.array(tf_tensor)
                
                # 标量处理
                if tf_tensor.shape == ():
                    tf_tensor = tf_tensor.reshape(1)
                    print(f"  ⚠ 标量转为1D张量: {tf_name}")
                ...
                print(f"  ✓ {tf_name} -> {pt_name}, shape: {list(pt_tensor.shape)}")
                
            except Exception as e:
                print(f"  ✗ 跳过 {tf_name}: {e}")
                continue
