    ...
    for key, column in input_columns.items():
        ...
        if column.get('is_sequence', False):
            ...
            if 'loss_condition' in column:
                ...
                # 这里需要根据condition['mask']来判断
                # 假设column有'loss_condition': {'key': 'type', 'mask': [T, F, T, ...]}
                # 表示哪些类型需要这个特征
...
def elem_masking(
    ...
    for key, column in input_columns.items():
        if not column.get('is_sequence', False):
            ...
            modified_inputs[key] = apply_token(inputs[key], column, selected_mask, 'masked')
...
def feat_masking(
    ...
    Returns:
        (modified_inputs, mfp_masks)
    """
    modified_inputs = {k: v.clone() for k, v in inputs.items()}
    mfp_masks = get_initial_masks(input_columns, mask)
